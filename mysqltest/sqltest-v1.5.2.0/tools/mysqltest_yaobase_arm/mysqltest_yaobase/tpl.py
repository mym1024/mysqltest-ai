...
# tpl = load_dir_vars('tpl')  # template of config file such as yaoadminsvr.conf/yaotxnsvr.conf...
...
ob5 = OBI('OBI_MASTER', ver='.30',
          ...
          # ms_port = 2944,
		    tpl = dict(schema_template=read('yubai.schema')),
                    ...
                    ups0 = YaoTxnSvr(ups0_ip, replay_checksum_flag=0),
...
ob6 = OBI('OBI_SLAVE', ver='.21',
                    rs0 = YaoAdminSvr(rs0_ip),
                    ups0 = YaoTxnSvr(ups0_ip, lsync_ip=ups0_ip, lsync_port=3046),
                    cs0 = YaoDataSvr(cs0_ip),
                    ms0 = YaoSqlSvr(ms0_ip),
...
${_ob_instances_cfg}
[root_server]
pid_file=log/yaoadminsvr.pid.$ip:$port
log_file=log/yaoadminsvr.log.$ip:$port
data_dir=data/as
log_level=$log_level
...
log_dir_path=data/as_commitlog
log_size=64
...
yaotxnsvr_template = '''
[public]
log_file = log/yaotxnsvr.log.$ip:$port
pid_file = log/yaotxnsvr.pid.$ip:$port
log_level = $log_level
trace_log_level=$trace_log_level
...
log_dir_path = data/ts_commitlog
...
log_file=log/yaosqlsvr.log.$ip:$port
pid_file=log/yaosqlsvr.pid.$ip:$port
log_level=$log_level
trace_log_level=$trace_log_level
...
yaodatasvr_template = '''
[public]
pid_file = log/yaodatasvr.pid.$ip:$port
log_file = log/yaodatasvr.log.$ip:$port
log_level = $log_level
trace_log_level=$trace_log_level
...
dev_name=$dev
port=$port
ups_commit_log_dir= data/ts_commitlog
