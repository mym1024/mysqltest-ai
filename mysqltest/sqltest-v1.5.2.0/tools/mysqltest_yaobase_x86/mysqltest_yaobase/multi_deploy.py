...
Examples:
  ...
  ## edit config.py.
  ./deploy.py yao1.update_local_bin
  ./deploy.py yao1[,.ct].reboot  # deploy and reboot yaoi and client, Will Cleanup Data
  ./deploy.py yao1[,.ct].restart # restart yaoi and client, Will Not Cleanup Data
  ./deploy.py yao1.random_test check 10 wait_time=1 # 'check' may be replaced by 'set_master/restart_server/disk_timeout/net_timeout/major_freeze/switch_schema/restart_client' or their combinations
  ./deploy.py yao1.random_test disk_timeout,set_master,major_freeze 10 wait_time=1 min_major_freeze_interval=10 # interval in seconds
  ./deploy.py yao1.switch_yaoi loop=10
  ./deploy.py yao1[,.ct].check_alive # check whether server and client are alive
  ./deploy.py yao1.ct.check_new_error
  ./deploy.py yao1[.ct,][stop,cleanup] # stop and cleanup yaoi and client
  ./deploy.py yao1.sysbench [prepare|run|cleanup] ['threads=1'] ['seconds=60'] ['requests=10000'] ['rows=10000'] # sysbench test
  ./deploy.py yao1.mysqltest [_quiet_=True] ['testset=test1[,test2[,...]]'] ['testpat=rowkey_*'] [record|test][disable-reboot]  # start mysqltest, more info plz dig into mysql_test dir
  ./deploy.py yao1.yaomysql [extra='extra-arguments-for-mysql-client'] [cmd]  # connect to oceabase by mysql client
...
More Examples:
  ./deploy.py yao1.[id,pid,version]  # view ip, port, ver, data_dir, pid
  ./deploy.py yao1.print_schema
  ./deploy.py yao1.rs0.[ip,port]
  ./deploy.py yao1.ups0.vi # view server config file
  ./deploy.py yao1.ups0.less # view log
  ./deploy.py yao1.ups0.grep
  ./deploy.py yao1.cs0.kill -41 # 41: debugon, 42: debugoff, 43: traceon, 44: traceoff
  ./deploy.py yao1.cs0.gdb
  ./deploy.py yao1.rs0.as_admin stat -o ups
  ./deploy.py yao1.ups0.ts_admin get_clog_cursor
  ./deploy.py yao1.ms[0,1,2].restart
  ./deploy.py yao1.ms0.ssh
  ./deploy.py yao1.ms0.ssh ps -uxf
  ./deploy.py yao[1,2,3,4].all_server_do kill_by_port
  ./deploy.py yao1.set_yaoi_role yaoi_role=YAOI_SLAVE
  ./deploy.py yao1.change_master_yaoi
  ./deploy.py yao1.[get_master_ups,sleep] _loop_=100 sleep_time=1
  ...
More Configuration:
...
def update_bin(src, dest, workdir, nccjyao=10):
    if not src: return 'no need to update_bin'
    server_list = '{yaoadminsvr,yaotxnsvr,yaodatasvr,yaosqlsvr}'
    server_list_in_src = '{yaoadminsvr/yaoadminsvr,yaotxnsvr/yaotxnsvr,yaodatasvr/yaodatasvr,yaosqlsvr/yaosqlsvr}'
 ...
 && rsync -a $workdir/home/fan/tools/yaobase/bin/$server_list $dest'''
        cmd = string.Template(cmd.replace('\n', '')).substitute(src=src, dest=dest, workdir=workdir, server_list=server_list, rpm='%s/yao.rpm'%(workdir))
...
def Role():
    ...
    set_yaoi_role = '''sup: $as_admin_cmd -r $ip -p $port set_yaoi_role -o $yaoi_role'''
    boot_strap_eof = '''sup: $localdir/tools/as_admin -t $boot_strap_timeout -r $ip -p $port boot_strap # ExceptionOnFail'''
    boot_strap = '''sup: $as_admin_cmd -t $boot_strap_timeout -r $ip -p $port boot_strap'''
    print_schema = '''sh: $as_admin_cmd -r $ip -p $port print_schema'''
    ...
    def start_new_way(**info):
        ...
        # add zhuyingying&linh 20200713
        usr_svr = sub2("$usr", info)
        ip_svr = sub2("$ip", info)
        dev = os.popen("ssh %s@%s /sbin/ip addr |grep %s | awk '{print $8}'" % (usr_svr, ip_svr, ip_svr)).read().strip('\n')
        #dev = os.popen("ssh %s@%s /sbin/ip route |sed -n '2p' | awk '{print $3}'" % (usr_svr, ip_svr, ip_svr)).read().strip('\n')
        if find_attr(info, 'tidy'):
            cmd = sub2(outline % "$exe", info)
        else:
            cmd = sub2({
                    "yaoadminsvr" : outline % "$exe -r ${rs0.ip}:${rs0.port} -R ${rs0.ip}:${rs0.port} -i %s -G $paxos_num -K $cluster_num -F 'false' -C $cluster_id -U $name -u $sex > /dev/null 2>&1" % (dev),
                    "yaotxnsvr" : outline % "$exe -r ${rs0.ip}:${rs0.port} -n ${app_name} -p $port -m $inner_port -i %s -g $paxos_id -C $cluster_id  > /dev/null 2>&1" % (dev),
                    "yaodatasvr" : outline % "$exe -r ${rs0.ip}:${rs0.port} -n ${app_name} -p $port -D ${cs_data_path:-$dir/data/cs} -i %s -C $cluster_id ${cs_extra_args} > /dev/null 2>&1" % (dev),
                    "yaosqlsvr" : outline % "$exe -r ${rs0.ip}:${rs0.port} -z $mysql_port -p $port -i %s -C $cluster_id ${ms_extra_args} > /dev/null 2>&1" % (dev),
    ...
    clear_clog = '''sh: ssh $usr@$ip rm -rf $dir/data/ts_commitlog/*'''
    clear_sstable = '''sh: ssh $usr@$ip rm -f $dir/data/ts_commitlog/log_replay_point $dir/data/ts_data/raid*/store*/*'''
    rsync2 = '''sh: rsync -a $_rest_ $usr@$ip:$dir/'''
    rsync_clog = '''sh: rsync -av $clog_src $usr@$ip: && ssh $usr@$ip cp $clog_src/* $dir/data/ts_commitlog # ExceptionOnFail'''
    touch_and_link_log_file = '''sh: ssh $usr@$ip 'touch $dir/log/$role.log.$ip:$port && ln $dir/log/$role.log{.$ip:$port,}' '''
    mkdir = '''sh: ssh $usr@$ip 'mkdir -p $dir/data/ts_data/raid{0,1} $dir/data/cs $dir/data/as $dir/log
mkdir -p $data_dir/$sub_dir_list/Recycle $data_dir/$sub_dir_list/$sub_data_dir/{sstable,ts_store,bypass} $clog_dir/$sub_data_dir/{as,ts}_commitlog/pool
mkdir -p $sstable_dir_list/$sub_data_dir/sstable
chown $$USER $data_dir/$sub_dir_list/$sub_data_dir -R
ln -sf $clog_dir/$sub_data_dir/as_commitlog $dir/data/
ln -sf $clog_dir/$sub_data_dir/ts_commitlog $dir/data/
ln -sf $sstable_dir_list $dir/data/cs/
for i in {0,1}; do for j in {0,1}; do [ -e $dir/data/ts_data/raid$$i/store$$j ] || ln -sf $data_dir/$$((1 + i * 2 + j))/$sub_data_dir/ts_store $dir/data/ts_data/raid$$i/store$$j;done; done' '''.replace('\n', '; ')
    ...
    ts_admin = '''sh: $ts_admin_cmd -a ${ip} -p ${port} -t $_rest_'''
    as_admin = '''sh: $as_admin_cmd -r ${ip} -p ${port} $_rest_'''
    ...
    minor_freeze = '''sh: $ts_admin_cmd -a ${ip} -p ${port} -t minor_freeze'''
    major_freeze = '''sh: $ts_admin_cmd -a ${ip} -p ${port} -t major_freeze'''
    ts_admin_p = '''popen: $ts_admin_cmd -a ${ip} -p ${port} -t $_rest_'''
    as_admin_p = '''popen: $as_admin_cmd -r ${ip} -p ${port} $_rest_'''
    reload_conf = '''sh: $ts_admin_cmd -a ${ip} -p ${port} -t reload_conf -f $dir/etc/$role.conf.$ip:$port'''
    roottable = '''popen: $localdir/tools/as_admin -r ${ip} -p ${port} print_root_table'''
    ...
    switch_schema = '''seq: rsync as_admin[switch_schema]'''
    return locals()

def role_op(yao, role, op, *args, **kw_args):
    def mycall(yao, path, *args, **kw_args):
        ret = call(yao, path, *args, **kw_args)
        return ret
    return [mycall(yao, '%s.%s'%(k, op), *args, **kw_args) for k, v in sorted(yao.items(), key=lambda x: x[0]) if type(v) == dict and re.match(role, v.get('role', ''))]

def yaoi_op(yaoi, op, *args, **attrs):
    return list_merge(role_op(yaoi, role, op, *args, **attrs) for role in 'yaoadminsvr yaotxnsvr yaosqlsvr yaodatasvr lsyncserver proxyserver'.split())
...
def ObInstance():
    ...
    is_finish_reporting = '''sh: $as_admin_cmd -r ${rs0.ip} -p ${rs0.port} stat -o rs_status|grep INITED'''
    is_bootstrap_ok = '''sh: ssh ${rs0.usr}@${rs0.ip} "grep 'set boot ok, all initial tables created' ${rs0.dir}/log/${rs0.role}.log"'''

    update_local_tools = '''sh: make tools$tool_ver/{iof,ts_admin,as_admin} src_dir=$src'''

    def test_bootstrap(cnt=10, **yao):
        gcfg['_quiet_'] = True
        print "begin boot strap testing, tyaoal cnt: [%s]" % cnt
        for i in range(int(cnt)):
            sys.stdout.write("The [%d] test: " % i)
            result = call_(yao, "reboot")
            if 0 == result[-1][-1]:
    ...
    def check_alive(**yao):
        return all(map(is_server_alive, get_match_child(yao, '.*svr$').values()))

    def check_basic(cli='ct', **yao):
        return call_(yao, 'check_alive') and call_(yao, '%s.check_alive'%(cli)) and call_(yao, '%s.check_new_error'%(cli))

    def require_start(**yao):
        if not check_until_timeout(lambda : call_(yao, 'is_finish_reporting', _quiet_=True) == 0 and IterEnd() or 'wait yao start service', 180, 1):
            raise Fail('yao not start before timeout!')
        return 'OK'

    def require_boot_ok(**yao):
        if not check_until_timeout(lambda : call_(yao, 'is_bootstrap_ok', _quiet_=True) == 0 and IterEnd() or 'wait yao boot ok', 180, 1):
            raise Fail('yao not boot ok before timeout!')
        return 'OK'

    def check_local_bin(**yao):
        if 0 != sh(sub2('ls  bin$ver/{$local_servers} lib/libsnappy_1.0.so tools$tool_ver/{$local_tools} >/dev/null', yao)):
            raise Fail('local bin file check failed. make sure "bin" "lib" "tools" dir and files exists!')
        return 'OK'
    def prepare_cs_data(cs_data='~/data', **yao):
        cs0 = call_(yao, 'cs0.ip');
        outline = '''ssh $usr@%s '(cp -r %s/* %s; sync )' '''
        if 0 != sh(sub2(outline % (cs0, cs_data, "$cs_data_path"), yao)):
            raise Fail('Prepare cs data failed!')
        return 'OK'
    def check_ssh(timeout=1, **yao):
        ip_list = set(sub2('$usr@$ip', v) for v in get_match_child(yao, '.+svr$').values())
    ...
    def check_dir(timeout=1, min_disk_space=1000, **yao):
        dir_list = set(sub2('$usr@$ip:$data_dir/$sub_dir_list', v) for v in get_match_child(yao, '.+svr$').values())
    ...
    def check_host(timeout=1, min_disk_space=1000, **yao):
        ssh_spec, data_dir_spec = sub2('$usr@$ip', yao), sub2('$usr@$ip:$data_dir/$sub_dir_list', yao)
    ...
    def set_yaoi_role_until_success(timeout=0.5, yaoi_role='YAOI_MASTER', **yao):
        if not check_until_timeout(lambda : call_(yao, 'rs0.set_yaoi_role', yaoi_role=yaoi_role) == 0 and IterEnd() or 'try set_yaoi_role', 30, timeout):
            return 'Fail[%s]'%(yaoi_role)
        return 'OK[%s]'%(yaoi_role)
    def boot_strap_maybe(yaoi_role='YAOI_MASTER', **yao):
        if yaoi_role == 'YAOI_SLAVE': return 'OK[SLAVE No need to bootstrap]'
        time.sleep(30)
        ret = call_(yao, 'rs0.boot_strap')
        time.sleep(30)
        if find_attr(yao, 'exception_on_bootstrap_fail') and ret != 0:
            raise Fail('bootstrap fail!', ret)
        #add liuxiao
        str_port=str(yao['mysql_port']);
        print(datetime.datetime.now())
        print 'echo "create database test" | mysql -h '+yao['ms0']['ip']+' -P'+str_port+' -uadmin -padmin;sleep 1;'
        p=Popen('echo "create database test" | mysql -h '+yao['ms0']['ip']+' -P'+str_port+' -uadmin -padmin;sleep 1;',shell=True, stdout=PIPE, stderr=PIPE)
        p=Popen('echo "set global yao_enable_query_optimizer=true;" | mysql -h '+yao['ms0']['ip']+' -P'+str_port+' -uadmin -padmin;sleep 1;',shell=True, stdout=PIPE, stderr=PIPE)
        p.wait()
        #add e
        return ret

    def switch_to_slave_yaoi_maybe(timeout=0.5, yaoi_role='YAOI_MASTER', **yao):
        if yaoi_role == 'YAOI_MASTER': return 'OK[this is YAOI_MASTER]'
        return call_(yao, 'switch_to_slave_yaoi')

    def get_master_ups(**yao):
        m = re.search('[^0-9.]([0-9.]+):(\d+)\(\d+\s+master', call_(yao, 'rs0.as_admin_p', 'stat -o ups'))
        if not m: return None
        return '%s:%s'% m.groups()

    def get_master_ups_name(**yao):
        ups = get_master_ups(**yao)
        master_ups = [k for k,v in get_match_child(yao, '^yaotxnsvr$').items() if type(v) == dict and sub2('$ip:$port', v) == ups]
        if not master_ups: return None
        else: return master_ups[0]

    def major_freeze(**yao):
        master_ups = get_master_ups_name(**yao)
        if not master_ups: raise Fail('no master ups present!')
        return call_(yao, '%s.major_freeze'%(master_ups))

    def all_server_do(*args, **yao):
        return par_do(yao, '.+svr', *args)

    def status(full=False, **yao):
        call_(yao, 'rs0.as_admin', 'get_yaoi_role')
        ups_list = get_match_child(yao, 'yaotxnsvr').keys()
        [call_(yao, '%s.ts_admin' %(ups), 'get_clog_status') for ups in ups_list]
        if full:
            call_(yao, 'rs0.as_admin', 'stat -o ups')
            call_(yao, 'rs0.as_admin', 'stat -o cs')
            call_(yao, 'rs0.as_admin', 'stat -o ms')

    def update_local_server(src=None, dest='bin$ver', workdir='$workdir/yao$ver', nccjyao=10, **yao):
        if src:
            return update_bin(sub2(src, yao), sub2(dest, yao), sub2(workdir, yao), nccjyao)
        else:
            return 'no src defined.'

    def do_cpbin(src=None, dest='bin$ver', workdir='$workdir/yao$ver', **yaom):
        server_list_in_src = '{yaoadminsvr/yaoadminsvr,yaotxnsvr/yaotxnsvr,yaodatasvr/yaodatasvr,yaosqlsvr/yaosqlsvr}'
    ...
    def update_faster(src=None, des='bin$ver', workdir='$workdri/yao$ver', nccjyao=10, **yao):
        server_list = '{yaoadminsvr,yaotxnsvr,yaodatasvr,yaosqlsvr}'
        server_list_in_src = '{yaoadminsvr/yaoadminsvr,yaotxnsvr/yaotxnsvr,yaodatasvr/yaodatasvr,yaosqlsvr/yaosqlsvr,lsync/lsyncserver}'
        if src:
            cmd = '''mkdir -p $workdir $des && make -j $nccjyao -C $src/src $servers && rsync -a $workdir/src/$server_list $des'''
            cmd = string.Template(cmd).substitute(src=sub2(src, yao), des=sub2(des, yao),
                                                  workdir=sub2(src, yao), server_list=server_list_in_src,
                                                  servers=server_list, nccjyao=nccjyao)
        else:
            return 'no src defined.'
        if gcfg.get('_verbose_', False):
            print cmd
    ...
    def get_master_ups_name2(**yao):
        ups = call_(yao, 'get_master_ups_using_client')
        master_ups = [k for k,v in get_match_child(yao, '^yaotxnsvr$').items() if type(v) == dict and sub2('$ip:$port', v) == ups]
        if not master_ups: return None
        else: return master_ups[0]

    def client(*req, **yao):
        cmd = 'log_level=INFO $client_cmd %(req)s rs=${rs0.ip}:${rs0.port} %(kw)s'%dict(req=' '.join(req), kw=kw_format(yao.get('__dynamic__', {})))
        return sh(sub2(cmd, yao))

    def yaomysql(*args, **yao):
        ori_cmd = 'mysql -h ${ms0.ip} -P ${mysql_port} -uadmin -padmin -c'
        if 'extra' in yao:
            ori_cmd += ' %s' % yao['extra']
        cmd = sub2(ori_cmd, yao)
        if 'cmd' in args:
            return cmd;
        else:
            return sh(cmd);

    # def sysbench(*args, **yao):
    ...
    #     opt["mysql-port"] = call_(yao, 'mysql_port')
    #     opt["mysql-host"] = call_(yao, 'ms0.ip')
    ...
    #         opt["command"] = "run"
    #     if "threads" in yao:
    #         opt["num-threads"] = yao["threads"]
    #     if "rows" in yao:
    #         opt["oltp-table-size"] = yao["rows"]
    #     if "seconds" in yao:
    #         opt["max-time"] = yao["seconds"]
    #     if "requests" in yao:
    #         opt["max-requests"] = yao["requests"]
    ...
    #     def do_after_test(mgr):
    #         call_(yao, 'collect_log', run_id="tablet_join_test")

    #     opt={}

    #     opt["port"] = call_(yao, 'mysql_port')
    #     opt["host"] = call_(yao, 'ms0.ip')
    #     opt["binary"] = call_(yao, 'localdir') + "/tools/yao_tablet_join_test"
    ...
    def update_cluster(*args, **yao):
        yao['extra'] = "-e\"replace into __all_cluster(cluster_vip, cluster_port,cluster_id,cluster_role,cluster_flow_percent) values('" + call_(yao, 'ms0.ip') + "', " + str(call_(yao, 'mysql_port'))+ " ,1,1,100);\""
        return call_(yao, "yaomysql")

    def quicktest(*args, **yao):
        gcfg['_quiet_'] = True
        mysql_test.pinfo('run libmysql quicktest ...')
        call_(yao, "mysqltest","quick","disable-reboot"); 
        mysql_test.pinfo('run libmysql+ps quicktest ...')
        call_(yao, "mysqltest","quick","disable-reboot","ps"); 
        mysql_test.pinfo('run java quicktest ...')
        call_(yao, "mysqltest","quick","disable-reboot","java"); 
        mysql_test.pinfo('run java+ps quicktest ...')
        call_(yao, "mysqltest","quick","disable-reboot","java","ps"); 
               

    def mysqltest(*args, **yao):
        def do_before_test(mgr):
            sh("sync")
            if need_reboot:
                reboot_again = True
                while reboot_again:
                    ...
                    ret = call_(yao, 'reboot')
                    if  ret[-1][-1] != 0:
                        force_info("reboot failed, retry after 60 seconds")
                        time.sleep(60)
                    else:
						# time.sleep(10)
                        call_(yao, "update_cluster")
                        reboot_again = False
            else:
                call_(yao, "update_cluster")
            os.putenv('OBMYSQL_PORT', str(mgr.opt["port"]))
            os.putenv('OBMYSQL_MS0', str(mgr.opt["host"]))
            os.putenv('RS0_IP', call_(yao, 'rs0.ip'))
            os.putenv('RS0_PORT', str(call_(yao, 'rs0.port')))
			#os.putenv('RS0_PORT', ‘29500’)
			
            master_ups = str(call_(yao, 'get_master_ups_name'))
            if master_ups == "ups0":
                os.putenv('NEW_MASTER_UPS_IP', str(call_(yao, 'ups1.ip')))
                os.putenv('NEW_MASTER_UPS_PORT', str(call_(yao, 'ups1.port')))
                os.putenv('MASTER_UPS_IP', str(call_(yao, master_ups + ".ip")))
                os.putenv('MASTER_UPS_PORT', str(call_(yao, master_ups + ".port")))
            else:
                os.putenv('NEW_MASTER_UPS_IP', str(call_(yao, 'ups0.ip')))
                os.putenv('NEW_MASTER_UPS_PORT', str(call_(yao, 'ups0.port')))
                os.putenv('MASTER_UPS_IP', str(call_(yao, master_ups + ".ip")))
                os.putenv('MASTER_UPS_PORT', str(call_(yao, master_ups + ".port")))

            os.putenv('UPS0_PORT', str(call_(yao, 'ups0.port')))
			#os.putenv('UPS0_PORT', ‘29700’)
            os.putenv('UPS0_IP', str(call_(yao, 'ups0.ip')))
            os.putenv('LOCAL_DIR', str(call_(yao, 'localdir')))
			#os.putenv('LOCAL_H', str(call_(yao, 'dir')))
        def do_after_test(mgr):
            if need_collect:
                call_(yao, 'collect_log', run_id=mgr.test)
            if gcfg['stop']:
                mgr.stop = True;
        ...
        if "disable-reboot" in args:
            need_reboot = False
        if "filter" in yao:
            opt["filter"] = yao["filter"]
        if "collect" in yao:
            if yao["collect"] == 'False':
                need_collect = False
        if "testset" in yao:
            opt["test-set"] = yao["testset"].split(',')
        if "testpat" in yao:
            # remove "test-set" that mysqltest'll ignore it
            if "test-set" in opt:
                del opt["test-set"]
            opt["test-pattern"] = yao["testpat"]
    ...
    def snapshot(*args, **yao):
        dir = call_(yao, 'dir')
        ms_log = os.path.relpath(call_(yao, 'ms0.log'), dir)
        rs_log = os.path.relpath(call_(yao, 'rs0.log'), dir)
        ups_log = os.path.relpath(call_(yao, 'ups0.log'), dir)
        cs_log = os.path.relpath(call_(yao, 'cs0.log'), dir)
        bkdir = "snapshot"
        ...
        cmd = "cd {0} && mkdir -p snapshot && tar -cvzf snapshot/{1} bin/yaodatasvr bin/yaoadminsvr bin/yaosqlsvr bin/yaotxnsvr {2} {3} {4} {5} {6}".format(dir, bkfile, core, ms_log, cs_log, ups_log, rs_log)
    ...
    def save_master_log(target='log', **yao):
        master = get_master_ups_name2(**yao)
        if not master: raise Fail('no master ups present!')
        return call_(yao, '%s.save_log'%(master), target)
    ...
    id = 'all: .+svr id'
    pid = 'par: .+svr pid'
    version = 'all: .+svr version'
    set_yaoi_role = 'call: rs0.set_yaoi_role'
    boot_strap = 'call: rs0.boot_strap'
    print_schema = 'call: rs0.print_schema'
    start_servers = 'par: .+svr start'
    # debugall = 'call: rs0.kill -41'
    start = 'seq: start_servers debugall'
    stop = 'par: .+svr kill_by_name'
    force_stop = 'par: .+svr kill_by_name -9'
    rsync = 'all: .+svr rsync'
    rsync_cs_sstable = 'all: yaodatasvr rsync_cs_sstable'
    conf = 'all: .+svr conf'
    mkdir = 'all: .+svr mkdir'
    cleanup = 'all: .+svr rmdir'
    collect_server_log = 'all: .+svr collect_log'
    ...
    ct_configure = 'all: .*clientset configure_yaoi'
    ct_prepare = 'all: .*clientset prepare'
    reload_conf = 'all: .+svr reload_conf'
    reconf = 'seq: conf rsync reload_conf'
    restart = 'seq: force_stop conf rsync mkdir ct_rsync start'
    touch_and_link_log_file = 'all: .+svr touch_and_link_log_file'
    exec_init_sql = '''sh: mysql -h ${ms0.ip} -P ${ms0.mysql_port} -uadmin -padmin -e "\. $init_sql_file" '''
    cs_data_prepare = 'seq: prepare_cs_data'
    switch_to_slave_yaoi = 'seq: set_yaoi_role_until_success[yaoi_role=YAOI_SLAVE] all_do[yaotxnsvr,kill_by_name,-9] all_do[yaotxnsvr,clear_clog] all_do[yaotxnsvr,start]'
    reboot = 'seq: check_local_bin check_ssh check_dir ct_check_local_file force_stop cleanup conf rsync mkdir ct_rsync ct_prepare rsync_cs_sstable tc_prepare touch_and_link_log_file start  set_yaoi_role_until_success boot_strap_maybe'
...
def ClientAttr():
    ...
    role = 'client'
    ssh = '''sh: ssh -t $ip $_rest_'''
    rsync = '''sh: cp ${yaoi.rs0.schema} $client/${yaoi.app_name}.schema; rsync -az $client $usr@$ip:$dir'''
...
def ClientSetAttr():
    ...
    reboot = 'seq: stop conf rsync clear yaoi.require_start require_start'
    restart = 'seq: stop conf rsync yaoi.require_start require_start'
    return locals()

role_vars = dict_filter_out_special_attrs(Role())
yaoi_vars = dict_filter_out_special_attrs(ObInstance())
...
def YaoAdminSvr(ip='127.0.0.1', port='$rs_port', **attr):
    return make_role('yaoadminsvr', ip=ip, port=port, role_data_dir='data/as_commitlog', **attr)

def YaoTxnSvr(ip='127.0.0.1', port='$ups_port', inner_port='$ups_inner_port', lsync_ip='', lsync_port='$default_lsync_port', master_ip='', master_port='$ups_port', **attr):
    return make_role('yaotxnsvr', ip=ip, port=port, inner_port=inner_port,
                     lsync_ip=lsync_ip, lsync_port=lsync_port, master_ip=master_ip, master_port=master_port,
                     role_data_dir='data/ts_commitlog data/ts_data/raid*/store*', **attr)
...
def YaoDataSvr(ip='127.0.0.1', port='$cs_port', **attr):
    return make_role('yaodatasvr', ip=ip, port=port, role_data_dir='data/cs/*/$app_name/sstable', **attr)

def YaoSqlSvr(ip='127.0.0.1', port='$ms_port', **attr):
    return make_role('yaosqlsvr', ip=ip, port=port, role_data_dir='', **attr)
...
def ObProxy(ip='127.0.0.1', port='$proxy_port', **attr):
    ...
    $exe -p $port --yaohost=${rs0.ip} --yaoport=${rs0.port} --loglevel=INFO --logpath=log/$role.log.$ip:$port \
    --yaologlevel=INFO --yaologpath=log/yaoapi.log $_rest_) >proxy.log 2>&1 &' '''
...
def YAOI(yaoi_role='YAOI_MASTER', port_suffix=None, masters=[], slaves=[], proxys=[], monitors=[], hosts=[], need_lsync=False, fake_ms=False, **attr):
    session = dict()
    yaoi_idx = ObCfg.yaoi_counter.next()
    if type(hosts) == dict: hosts.update(cfg_cache_path=ObCfg.cfg_cache_path, dir='yao%d'%(yaoi_idx))
    if not port_suffix: port_suffix = yaoi_idx
    ...
    if not get_match_child(attr, '^yaoadminsvr$'):
        ...
        for idx, master in enumerate(masters):
            deried_vars['rs%d'%(idx)] = YaoAdminSvr(master)
            deried_vars['ups%d'%(idx)] = YaoTxnSvr(master)
            if fake_ms:
                deried_vars['fake_ms%d'%(idx)] = YaoSqlSvr(master, mysql_port=2828)
            if need_lsync:
                deried_vars['lsync%d'%(idx)] = LsyncServer(master)
        for idx, slave in enumerate(slaves):
            deried_vars['cs%d'%(idx)] = YaoDataSvr(slave)
            deried_vars['ms%d'%(idx)] = YaoSqlSvr(slave)
    ...
    yaoi = dict_updated(yaoi_vars, yaoi_role=yaoi_role, hosts=hosts, **dict_merge(deried_vars, attr))
    if not get_match_child(yaoi, '^.*svr$'):
        raise Fail('yao%d: not even one server defined: default_hosts=%s!'%(yaoi_idx, ObCfg.default_hosts))
    if (any((v.get('client') == 'bigquery' or v.get('client') == 'sqltest' ) for v in get_match_child(attr, 'client').values()) and not get_match_child(yaoi, '^proxyserver$')):
        raise Fail('yao%d: no proxy server defined, but you request use "bigquery" or "seqltest" as client'%(yaoi_idx))
    yaoi.update(local_servers = ObCfg.local_servers)
    if get_match_child(yaoi, '^lsyncserver$'):
        yaoi['local_servers'] += ',lsyncserver'
    if get_match_child(yaoi, '^proxyserver$'):
        yaoi['local_servers'] += ',proxyserver'
    if get_match_child(yaoi, '^monitorserver$'):
        yaoi['local_servers'] += ',monitorserver'
    for ct in get_match_child(yaoi, '^.*clientset$').values():
        ct.update(yaoi=yaoi)
    def AfterLoadHook(yaocfg):
        saved_quiet_value = gcfg.get('_quiet_')
        gcfg.update(_quiet_=True)
        call(yaoi, "ct_configure")
...
class ObCfg:
    def all_do(pat, method, *arg, **yao):
        return par_do(yao, pat, method, *arg)

    def sleep(*arg, **yao):
        sleep_time = float(yao.get('sleep_time', 1))
    ...
    yaoi_counter = itertools.count(1)
    ...
    local_servers = 'yaoadminsvr,yaotxnsvr,yaosqlsvr,yaodatasvr'
    local_tools = 'as_admin,ts_admin'
    as_admin_cmd = '$localdir/tools$ver/as_admin'
    ts_admin_cmd = '$localdir/tools$ver/ts_admin'
    ...
    def get_cfg(path):
        ...
        [v.get('name') or v.update(name=k) for k,v in get_match_child(cfg, 'yaoi').items()]
        return cfg
    @staticmethod
    def get_cfg_(path):
...
if __name__ == '__main__':
    ...
    if method.find("dosql") != -1:
        gcfg['_quiet_'] = True
    try:
        yao_cfg = ObCfg.get_cfg(config)
    except Fail as e:
        print e
        sys.exit(1)
    yao_cfg = add_parent_link(yao_cfg)
    yao_cfg.update(help=lambda *ls,**kw: usages())
    if method.find("list") != -1:
        gcfg['_quiet_'] = True
        pref = len(rest_args) and len(rest_args[0]) and rest_args[0] or ""
        pref = re.sub('.?[^.]*$', '', pref)
        yaoj = find_attr(yao_cfg, pref)
        if type(yaoj) == dict:
            margs = [pref and "%s.%s" % (pref,k) or k \
                         for k in yaoj.keys() if not re.match('^__', k)]
            print(" ".join(margs))
            # for k in yaoj.keys():
    ...
    for i in range(_loop_):
        ...
        try:
            for m in multiple_expand(method):
                #info(pformat(dict_strip(call(yao_cfg, m, *rest_args, **kw_args))))
                key, result = call(yao_cfg, m, *rest_args, **kw_args)
